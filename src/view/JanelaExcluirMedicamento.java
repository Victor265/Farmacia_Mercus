...
import control.ExcluirMedicamentos;

public class JanelaExcluirMedicamento extends JFrame implements ActionListener {
	...
	public JanelaExcluirMedicamento(String titulo) {
		super("Farmácia Mercus - Excluir Medicamentos");
		...
		this.jlCabecalho = new JLabel("Excluir através do Código");
		...
		this.btOk.setToolTipText("Excluir");
	...
	public void actionPerformed(ActionEvent evento) {
		if (evento.getSource() == this.btOk) {
			ExcluirMedicamentos excMedicamento = new ExcluirMedicamentos();
			excMedicamento.excluiMedicamento(jtCodigo.getText(), conexao.getConexao());
			dispose();
		}
		...
		if (evento.getSource() == this.btVoltar) {
			dispose();
			JanelaExcluirCliente jcc = new JanelaExcluirCliente(
